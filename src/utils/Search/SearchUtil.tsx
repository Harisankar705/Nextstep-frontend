...
const SearchUtil: React.FC<SearchBarProps> = ({
  ...
  const [searchResults, setSearchResults] =
    useState<SearchResults>(INITIAL_RESULTS);
        ...
        {user.profilePicture ? (
          <img
            ...
            onError={(e) => {
              ...
              if (parent) {
                parent.innerHTML =
                  '<div class="text-gray-400"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="5"/><path d="M20 21a8 8 0 0 0-16 0"/></svg></div>';
              }
            }}
          />
        ) : (
        ...
        {employer.logo ? (
          <img
            ...
            onError={(e) => {
              ...
              if (parent) {
                parent.innerHTML =
                  '<div class="text-gray-400"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V5z"/></svg></div>';
              }
            }}
          />
        ) : (
  ...
  const renderResultSection = <T extends UserCandidate | Employer | PostType>(
    title: string,
    items: T[],
    type: "users" | "employers" | "posts"
  ) => {
    if (items.length === 0) return null;
    return (
      <div className="border-t border-gray-100 first:border-0">
        <h3 className="font-semibold text-sm text-gray-600 px-4 py-2 sm:px-3 sm:py-2">
          {title}
        </h3>
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="w-full min-w-[240px] sm:min-w-[200px] md:min-w-[250px] lg:min-w-[300px] pl-9 pr-3 py-2 bg-gray-900 border border-gray-800 rounded-full 
      ...
      {showResults && (
        <div
          className="absolute left-0 right-0 top-full mt-2 bg-white rounded-lg shadow-lg 
                      border border-gray-200 max-h-[400px] sm:max-h-[300px] md:max-h-[350px] lg:max-h-[400px] overflow-y-auto z-50"
        >
          {isSearching ? (
