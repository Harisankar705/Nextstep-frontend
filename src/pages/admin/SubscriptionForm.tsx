...
export const SubscriptionForm = () => {
  ...
  const validateForm = () => {
    ...
    // Validate validity
    if (!plan.validity.trim()) {
      newErrors.validity = "Validity period is required";
    } else if (!plan.validity.includes("day") && !plan.validity.includes("month") && !plan.validity.includes("year")) {
      newErrors.validity = "Validity should include units (days, months, years)";
    }
    
    // Validate features
    const emptyFeatures = plan.features.filter(feature => !feature.trim());
  ...
  const handleFeatureChange = (index: number, value: string) => {
    ...
    // Clear feature errors when user makes changes
    if (errors.features) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error?.message || (id ? "Failed to update subscription!" : "Failed to create subscription!"));
    } finally {
        ...
        {isLoading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={plan.price || 0}
                    onChange={handleInputChange}
                    className={`w-full pl-8 pr-3 py-2 border ${errors.price ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500`}
                    min="1"
                    step="0.01"
                    placeholder="0.00"
                    required
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  value={plan.validity || ""}
                  onChange={handleInputChange}
                  className={`w-full px-3 py-2 border ${errors.validity ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500`}
                ...
                {plan.features.map((feature, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <input
                      ...
                      className={`flex-1 px-3 py-2 border ${errors.features ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500`}
            ...
            <div className="flex justify-end">
              ...
              >
                {isSubmitting ? "Processing..." : (id ? "Update Plan" : "Create Plan")}
              </button>
            </div>
          </form>
        )}
